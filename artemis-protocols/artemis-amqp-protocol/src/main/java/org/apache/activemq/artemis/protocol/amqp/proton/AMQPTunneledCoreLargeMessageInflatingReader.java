 ...
 */
public class AMQPTunneledCoreLargeMessageInflatingReader implements MessageReader {
   ...
   private final byte[] scratchBuffer = new byte[8192];
   ...
   public AMQPTunneledCoreLargeMessageInflatingReader(ProtonAbstractReceiver serverReceiver) {
   ...
   @Override
   public AMQPTunneledCoreLargeMessageInflatingReader open() {
      if (state != State.CLOSED) {
   ...
   private boolean tryReadHeadersAndProperties(ByteBuf buffer) throws Exception {
      ...
      try {
         ...
         coreMessage.decodeHeadersAndProperties(inflateBuffer(coreHeadersBuffer));
   ...
   private ByteBuf inflateBuffer(ByteBuf deflatedBytes) throws Exception {
      final ByteBuffer input = deflatedBytes.nioBuffer();
   ...
   private void tryReadMessageBody(Delivery delivery, ByteBuf buffer) throws Exception {
      ...
      final ByteBuffer input = buffer.nioBuffer(buffer.readerIndex(), writeSize);
      ...
      final ActiveMQBuffer chunk = ActiveMQBuffers.dynamicBuffer(scratchBuffer.length);

      while (!inflater.finished() && !inflater.needsInput()) {
         int decompressedSize = inflater.inflate(scratchBuffer);
         chunk.writeBytes(scratchBuffer, 0, decompressedSize);
      }

      try {
         coreLargeMessage.addBytes(chunk);
      } catch (ActiveMQException ex) {
         throw ex;
      } catch (Exception ex) {
